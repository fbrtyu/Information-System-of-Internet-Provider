...
func SetInfo(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec("UPDATE client SET MobilePhone = ? WHERE ID = ?", r.FormValue("Mobile"), user.IDClient)
	if err != nil {
		panic(err)
	}

	_, err = db.Exec("UPDATE client SET Email = ? WHERE ID = ?", r.FormValue("Email"), user.IDClient)
	if err != nil {
		panic(err)
	}
